...
def json_saver(state: AgentState, config):
    ...
    if content_only:
        ...
        # Add user_id and save to DynamoDB
        user_id = state.get("user_id", "default_user")  # Retrieve user_id from state or use default
        dynamodb_item = {
            "user_id": user_id,
            "data": content_only
        }
        table.put_item(Item=dynamodb_item)
        print(f"Data successfully saved to DynamoDB under user_id: {user_id}.")
